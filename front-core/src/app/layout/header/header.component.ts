  ...
  implements OnInit, AfterViewInit
{
  ...
  ngOnInit() {
    ...
    const userRole = this.authenticationService.getUser().userType;
    ...
    this.practiceName = user.practiceName;
    this.colorHex1 = user.practiceColorHex1;
    this.practiceManagementService.getLogo(user.practiceId).subscribe((response: any) => {
      if (response.size !== 0) {
        this.practiceLogo = this.sanitizer.bypassSecurityTrustUrl(URL.createObjectURL(response));
      }
    });
    ...
    } else if (userRole === "Practice manager") {
